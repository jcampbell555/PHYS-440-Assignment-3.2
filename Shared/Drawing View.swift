...
/////
//func CesaroFractalCalculator (x: CGFloat, y: CGFloat, size: Double, angleDivisor: Int) -> [(xPoint: Double, yPoint: Double)]
//{
...
//    // Calculates Side 1
//    angleChange = 90.0
//    allThePoints += calculateCesaroSide(&angle, &angleChange, &myX, &myY, size, angleDivisor)
//
//    // Calculates Side 2
//    angleChange = -90.0
//    allThePoints += calculateCesaroSide(&angle, &angleChange, &myX, &myY, size, angleDivisor)
...
/////
//func calculateCesaroSide(_ angle: inout CGFloat, _ angleChange: inout CGFloat, _ myX: inout CGFloat, _ myY: inout CGFloat, _ size: Double, _ angleDivisor: Int) -> [(xPoint: Double, yPoint: Double)] {
//
//    var currentPoint: [(xPoint: Double, yPoint: Double)] = []  ///Array of tuples
//    ///
//    angle = turn(angle: angle, angleChange: angleChange)
//    currentPoint += CesaroSide(fractalnum: Int(0.0), x: myX, y: myY, angle: angle, size: size, divisorForAngle: angleDivisor)
...
func ParticleWalk (EnergyI: Int, EnergyLoss: Int, meanFP: Int, InitH: Double, boxSizeX: Double, boxSizeY: Double)-> [(xPoint: Double, yPoint: Double)] {

    //sets energy to initial energy
    var Energy = EnergyI
    ...
    //creates random direction in relation to unit circle
    var RandDir = Double.random(in: 0..<2*Double.pi)

    var newPosX = InitX
    var newPosY = InitY
    var finalPos = [(xPoint: Double, yPoint: Double)]().self

    //while the particle is above a certain energy, continue doing this
    while (Double(Energy) > 1E-5){

        // changes new position to equal the x and y of the random direction
        newPosX = InitX + (Double(meanFP) * cos(RandDir))

        newPosY = InitY + (Double(meanFP) * sin(RandDir))
        //reduces the energy by a user-defined value
        Energy = Energy - EnergyLoss

        //creates a new random direction
        RandDir = Double.random(in: 0..<2*Double.pi)
        
        finalPos.append((xPoint: newPosX, yPoint: newPosY))

        // tells us if the position of the particle is outside the bounds of the walls it should stop
        if (newPosX < -boxSizeX || newPosX > boxSizeX || newPosY < -boxSizeY || newPosY > boxSizeY){
            Energy = Int(0.0)
            
            
        }

//        finalPos += [(xPoint: newPosX, yPoint: newPosY)]
